...
for (mtd in allmtd){
  ...
  if (mtd=='AcImpute'||mtd=='Acmagic'||mtd=='magic'||mtd=="deepimpute"||mtd=='Acmagic_np'){
...
p
ggsave(p,filename = "E:/学习箱/论文/2/cor.pdf",width = 12,height = 9)
...
for (i in 1:length(datasets)) {
  eval(parse(text = paste0("Acmagic<-read.csv('E:/学习箱/论文/2/datasets/Ziegenhain/",datasets[i],"_Acmagic_np.csv',header = T)")))
  
  # eval(parse(text = paste0("Acmagic<-read.csv('E:/学习箱/论文/2/datasets/Ziegenhain/",datasets[i],"_procv_nor_Acmagicrenormalt0.csv',header = T)")))
  ...
  erccAcmagic = Acmagic[grep('ERCC',Acmagic[,1]),]
  common_rows <- intersect(erccraw[, 1], erccAcmagic[, 1])
  
  erccAcmagic = erccAcmagic[erccAcmagic[,1]%in%common_rows,]
  erccraw = erccraw[erccraw[,1]%in%common_rows,]
  ercc = ercc[ercc[,1]%in%gsub("g", "", common_rows),]
  erccAcmagic = erccAcmagic[order(erccAcmagic[,1]),]
  erccraw = erccraw[order(erccraw[,1]),]
  ercc = ercc[order(ercc[,1]),]
  
  erccAcmagic  = apply(erccAcmagic[,-1],2,as.numeric)
  ercc = as.numeric(ercc[,2])
  
  erccrawcor = corercc(log(ercc),log(erccraw[,-1]+1))
  erccAcmagic = corercc(log(ercc),log(erccAcmagic+1))
  
  dataercc=cbind(erccrawcor,erccAcmagic)
  dataercc=data.frame(dataercc)
  colnames(dataercc)<-c("raw","Acmagic")
  dataercc <- dataercc %>% 
  ...
  #内容没有 格式转换
  # InsectSprays %>% {
  eval(parse(text = paste0("p",datasets[i],"<- ggplot(dataercc,mapping = aes(x = '',y = correlation , fill = factor(Ziegenhain,levels = c('raw','Acmagic')) ))+
